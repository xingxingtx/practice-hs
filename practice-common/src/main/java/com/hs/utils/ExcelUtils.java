...
import com.hs.annotation.Excel;
...
public class ExcelUtils<T> {
    ...
    public List<T> importExcel(Class<T> tClass, String sheetName, InputStream input, int rowNum) throws Exception {
        ...
        if(sheets <= 0){
            log.error("sheets number is :{}", sheets);
            return list;
        }
        //判断rowNum 是否是>0 小于0 则默认数据行位第二行，第一行默认为表头行
        ...
        for (int index = 0; index < sheets; index++) {
            ...
            if(fieldsList == null){
                fieldsList = getFieldsList(tClass, sheet.getRow(titleNum));
            }
            //获取数据总行数
    ...
    private List<Field> getFieldsList(Class<T> tClass, Row row) {
        //表头和字段对应的关系
        Map<String, Field> filedMappings = new HashMap<>(16);
        // 有数据时才处理 得到类的所有field.
        ...
        for (int col = 0; col < allFields.length; col++) {
            ...
            if (field.isAnnotationPresent(Excel.class)) {
                ...
                filedMappings.put(annotation.name(), field);
            }
        }
        //获取总cells数量
        int cells = row.getPhysicalNumberOfCells();
        List<Field> fields = new ArrayList<>(cells);
        for (int i = 0; i < cells; i++) {
            fields.add(filedMappings.get(row.getCell(i).getStringCellValue().trim()));
        }
        return fields;
    }
